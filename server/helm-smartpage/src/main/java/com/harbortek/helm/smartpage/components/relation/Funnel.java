...
@Service
public class Funnel implements Component {
    @Autowired
    ...
    public String getData(Long pageId, DataRequest request) {
        ...
        if (dimensionFields.size() == 1 && valueFields.size() == 1) {
    ...
    private JSONArray buildSingleSeriesData(SqlRowSet rowSet, DatasetField dimensionField, DatasetField valueField) {
        ...
        while (rowSet.next()) {
            ...
            Object xValue = String.valueOf(rowSet.getObject(dimensionField.getName()));
            row.set("name", xValue);
