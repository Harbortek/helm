...
package com.harbortek.helm.smartpage.components.trend;
...
@Service
public class LineStack implements Component {
    ...
    public String getData(Long pageId, DataRequest request) {
        ...
        if (dimensionFields.isEmpty() || valueFields.isEmpty()) {
            //空数据
            data = new JSONArray();
        } else if (stackFields.isEmpty() && dimensionFields.size() == 1 && valueFields.size() == 1) {
            //单线图
            data = buildSingleSeriesData(rowSet, dimensionFields.get(0), valueFields.get(0));
        } else if (stackFields.size() == 1 && valueFields.size() == 1) {
            //多序列图，以堆叠维度为序列
            data = buildMultiDimensionSeriesData(rowSet, dimensionFields.get(0), stackFields.get(0),
                                                 valueFields.get(0));
        } else if (stackFields.isEmpty() && valueFields.size() > 1) {
            //多序列图,以值为序列
