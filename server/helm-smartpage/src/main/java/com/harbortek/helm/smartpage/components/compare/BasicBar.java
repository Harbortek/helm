...
@Service
public class BasicBar implements Component {
    ...
    public String getData(Long pageId, DataRequest request) {
        ...
        if (dimensionFields.isEmpty() || valueFields.isEmpty()) {
            //空数据
            data = new JSONArray();
        } else if (dimensionFields.size() == 1 && valueFields.size() == 1) {
        ...
        } else if (dimensionFields.size() == 2 && valueFields.size() == 1) {
            SqlRowSet rowSet = datasetService.groupBy(pageId, dataset, request.getParams(), groupByFields, valueFields,
                                                      filterFields);
            //多序列图，以子维度为序列
            data = buildMultiDimensionSeriesData(rowSet, dimensionFields.get(0), dimensionFields.get(1),
                                                 valueFields.get(0));
