...
@Service
public class Liquid implements Component {
    @Autowired
    ...
    public String getData(Long pageId, DataRequest request) {
        ...
        if ("dynamic".equalsIgnoreCase(size.getStr("liquidMaxType"))){
            JSONObject liquidMaxField = size.getJSONObject("liquidMaxField");
            String fieldName = liquidMaxField.getStr("id");
            String summaryMethod = liquidMaxField.getStr("summary");
