...
@Service
public class BidirectionalBar implements Component {
    @Autowired
    ...
    public String getData(Long pageId, DataRequest request) {
        ...
        List<DatasetField> secondValueFields = config.getYaxisExt();
        ...
        fields.addAll(secondValueFields);

        List<DatasetField> groupByFields = new ArrayList<>();
        groupByFields.addAll(dimensionFields);

        List<DatasetField> fullValueFields = new ArrayList<>();
        fullValueFields.addAll(valueFields);
        fullValueFields.addAll(secondValueFields);

        JSONObject result = new JSONObject();
        result.set("fields", fields);

        JSONArray data = new JSONArray();
        if (dimensionFields.size() == 1 && valueFields.size() == 1 && secondValueFields.size()==1) {

            SqlRowSet rowSet = datasetService.groupBy(pageId, dataset, request.getParams(), groupByFields, fullValueFields,
                                                      filterFields);
            //单线图
            data = buildSingleSeriesData(rowSet, dimensionFields.get(0), valueFields.get(0),secondValueFields.get(0));
        }
    ...
    private JSONArray buildSingleSeriesData(SqlRowSet rowSet, DatasetField dimensionField, DatasetField valueField,
        ...
        while (rowSet.next()) {
            ...
            row.set("extValue", rowSet.getBigDecimal(extValueField.getName()));
