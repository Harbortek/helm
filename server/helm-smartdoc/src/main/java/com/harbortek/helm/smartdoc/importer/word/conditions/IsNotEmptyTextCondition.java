...
@ParagraphCondition(id= ConditionTypes.IS_NOT_EMPTY)
public class IsNotEmptyTextCondition implements IParagraphCondition {
    public IsNotEmptyTextCondition() {
    }

    public boolean isSatisfied(Element paragraph, String argument) {
        String text = WordUtils.plainText(paragraph);
        return StringUtils.isNotEmpty(StringUtils.trimToEmpty(text));
    }
