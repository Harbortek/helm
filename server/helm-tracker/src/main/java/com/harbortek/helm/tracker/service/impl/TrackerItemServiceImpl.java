...
public class TrackerItemServiceImpl implements TrackerItemService {
    ...
    public Page<TrackerItemVo> findTrackerItems(Long projectId, Long trackerId, Long sprintId, ObjectFilter filter,
        ...
        List<Long> filterTrackerIds = getFilterTrackerId(projectId, trackerId, sprintId, isTest);
        List<Long> sprintIds = sprintDao.findSprintByProjectId(projectId).stream().map(BaseEntity::getId).toList();

        Page<TrackerItemEntity> entities =
                trackerItemDao.findTrackerItems(projectId, trackerId, sprintId, filter, keyword,filterTrackerIds,sprintIds,pageable);
        if(entities.isEmpty()){
...
//    }
    ...
    public Page<TrackerItemVo> findTrackerItemsTree(Long projectId, Long trackerId, Long sprintId, ObjectFilter filter,
        ...
        List<Long> sprintIds = sprintDao.findSprintByProjectId(projectId).stream().map(BaseEntity::getId).toList();
        Page<TrackerItemEntity> entities =
                trackerItemDao.findTrackerItems(projectId, trackerId, sprintId, filter, keyword, null,sprintIds, pageable);
...
//                item.getId(),"","没有修改工作项权限");
    ...
    public void updateTrackerItemSprint(Long sprintId, List<Long> trackerItemIds) {
        ...
        for (TrackerItemEntity itemEntity : trackerItemEntities) {
            ...
            //记录日志
            changeLogDao.createChangeLog(itemEntity.getId(), ChangeLogMessages.TRACKER_ITEM_FIELD_CHANGE, "所属迭代",
                    itemEntity, oldValue.get(), newValue);
            ...
            if (ObjectUtils.isNotEmpty(trackerEntity)) {
                ...
                trackerNotificationService.sendSystemNotification(NotificationEvents.CHANGE_ITEM_SPRINT,
                        trackerEntity, itemEntity, SecurityUtils.getCurrentUser());
            }
...
//    }
    ...
    public List<FilterCondition> findGroupItems(Long projectId, Long trackerId, Long sprintId, ObjectFilter filter, String keyword) {
        ...
        return trackerItemDao.findGroupItems(projectId, trackerId, sprintId, filter, keyword,sprintIds);
    }
