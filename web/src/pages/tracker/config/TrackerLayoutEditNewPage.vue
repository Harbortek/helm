<template>
    <config-page :routes="routes()" style="padding:0" title="" description="">
        <a-layout style="">
            ...
            <a-layout-content class="main-content">
                <div class="main-wrapper">
                    <tracker-layout-edit-new :tracker="tracker" :customerFields="customerFields" :sections="sections"
                        :selectTab="selectTab" @changeSelectTab="onChangeSelectTab"></tracker-layout-edit-new>
                </div>
                <div class="main-right-wrapper">
                    <span v-if="isCustomConfig" @click="selectTab = 'tab'"
                        :class="{ 'clickable-zoom': true, 'box-wrap': true, actived: selectTab == 'tab' }"></span>
                    <div class="modal-wrapper">
                        <h3>模块标签页</h3>
                        <a-list>
                            <a-list-item v-for="item in sections" :key="item.value"
                                style="border:none;padding:6px 0">{{ item.name }}</a-list-item>
                        </a-list>
                    </div>
                </div>
            ...
            <a-layout-sider class="right-sider-content" style="max-width: 300px;min-width: 240px;">
                ...
                <template v-else-if="selectTab == 'tab'">
                ...
                <template v-else>
                    <h3>操作引导</h3>
                    <div class="guide-warp" style="">
                        <h4>1.标签页配置</h4>
                        <span style="margin-left:10px;">选中侧栏，以查看或编辑模块标签页。</span>
                        <div class="guide-img">
                            <img style="object-fit: cover;width: 100%;height: 100%;"
                                :src="require('@/assets/layout/layout-new01.png')" />
                        </div>
                        ...
                        <div class="guide-img">
                            <img style="object-fit: cover;width: 100%;height: 100%;"
                                :src="require('@/assets/layout/layout-new02.png')" />
                        </div>
                    </div>
                </template>
        ...
        <tracker-layout-Key-fields-dialog :isShowDialog="keyFieldsVisible" :tracker="tracker"
            :customerFields="customerFields" @cancel="keyFieldsVisible = false"></tracker-layout-Key-fields-dialog>
            ...
            <a-form-model :model="formData">
                <a-form-model-item prop="field">
                    <a-select placeholder="选择属性" mode="multiple" style="width: 460px;" show-search optionFilterProp='children'
                        ...
                        <a-select-option v-for="item in addFields" :key="item.id"
...
export default {
    ...
    components: {
        ...
        TrackerLayoutKeyFieldsDialog, TrackerLayoutEditNew
    },
    data() {
        return {
            tableHeight: 400,
    ...
    methods: {
        ...
        onClickRelease() {
            let layout = this.findLayout('NEW')
            ...
            updateTrackerLayout(this.tracker.id, layout).then(resp => {
                ...
                this.$router.push("trackerLayoutConfig")
            })
        },
        onClickCancel() {
            let that = this
        ...
        onOKAddFields() {
            if (this.formData.field) {
                let fieldList = this.findFieldList(this.formData.field)
                if (fieldList) {
                    this.customerFields.push(...fieldList)
                }
        ...
        routes: function () {
            return [{
                ...
                name: 'trackerLayoutEditNew', meta: {
                    icon: "blank",
        ...
        rowDrop(visiable) {
            ...
            this.$nextTick(() => {
                ...
                if (this.sortable && this.sortable.option("animation") == '151') {
