      ...
      <a-form-model ref="axisForm" :model="axisForm" label-width="80px" :layout="'horizontal'" :labelCol="{ span: 7 }"
        ...
        <div v-show="axisForm.show">
          <a-form-model-item v-show="showProperty('position')" :label="$t('chart.position')" class="form-item">
            <a-radio-group v-model="axisForm.position" size="small" @change="changeXAxisStyle('position')">
              ...
              <div v-else-if="chart.type === 'bidirectional-bar'">
                <a-radio-button value="top">{{ $t('chart.text_pos_left') }}</a-radio-button>
                <a-radio-button value="bottom">{{ $t('chart.text_pos_center') }}</a-radio-button>
              </div>
            </a-radio-group>
          </a-form-model-item>
          <a-form-model-item v-show="showProperty('name')" :label="$t('chart.name')" class="form-item">
            <a-input v-model="axisForm.name" size="small" @blur="changeXAxisStyle('name')" />
          </a-form-model-item>
          ...
          <span v-show="showProperty('axisValue')">
            ...
            <span v-show="!axisForm.axisValue.auto">
              ...
              <a-form-model-item :label="$t('chart.axis_value_split_count')" class="form-item">
                ...
                <a-input v-model="axisForm.axisValue.splitCount" @blur="changeXAxisStyle('axisValue')" />
              </a-form-model-item>
            </span>
          </span>
          ...
          <a-divider v-if="showProperty('axisForm')" />
          <a-form-model-item v-show="showProperty('axisForm')" :label="$t('chart.axis_label_show')" class="form-item">
            ...
            <span v-show="chart.type && chart.type.includes('horizontal')">
              ...
              <a-form-model-item v-show="axisForm.axisLabelFormatter.type !== 'percent'"
                ...
                <a-select v-model="axisForm.axisLabelFormatter.unit" :placeholder="$t('chart.pls_select_field')"
                  dropdownClassName="props-dropdown" size="small" @change="changeXAxisStyle('axisLabelFormatter')">
                  <a-select-option v-for="item in unitList" :key="item.value" :value="item.value">{{ $t('chart.' +
