    ...
    <a-col style="flex: 1 1;">
      <a-form-model ref="axisForm" :model="axisForm" label-width="80px" :layout="'horizontal'" :labelCol="{ span: 7 }"
        ...
        <a-form-model-item v-show="showProperty('show')" :label="$t('chart.show')" class="form-item">
          <a-checkbox v-model="axisForm.show" @change="changeYAxisStyle('show')">{{ $t('chart.show') }}</a-checkbox>
        </a-form-model-item>
        <div v-show="showProperty('show') && axisForm.show">
          <a-form-model-item v-show="showProperty('position')" :label="$t('chart.position')" class="form-item">
            <a-radio-group v-model="axisForm.position" size="small" @change="changeYAxisStyle('position')">
          ...
          <a-form-model-item v-show="showProperty('name')" :label="$t('chart.name')" class="form-item">
            <a-input v-model="axisForm.name" size="small" @blur="changeYAxisStyle('name')" />
          </a-form-model-item>
          <a-form-model-item v-show="showProperty('nameTextStyle')" :label="$t('chart.axis_name_color')"
            ...
            <vue-color-picker v-model="axisForm.nameTextStyle.color" class="color-picker-style"
              :predefine="predefineColors" @change="changeYAxisStyle('nameTextStyle')" />
          </a-form-model-item>
          <a-form-model-item v-show="showProperty('nameTextStyle')" :label="$t('chart.axis_name_fontsize')"
            ...
            <a-select v-model="axisForm.nameTextStyle.fontSize" :placeholder="$t('chart.axis_name_fontsize')"
              dropdownClassName="props-dropdown" @change="changeYAxisStyle('nameTextStyle')">
          ...
          <span v-show="showProperty('axisValue')">
            ...
            <a-form-model-item class="form-item">
              ...
              <a-checkbox v-model="axisForm.axisValue.auto" @change="changeYAxisStyle('axisValue')">{{
            ...
            <span v-show="!axisForm.axisValue.auto">
              <a-form-model-item :label="$t('chart.axis_value_min')" class="form-item">
                <a-input v-model="axisForm.axisValue.min" @blur="changeYAxisStyle('axisValue')" />
              </a-form-model-item>
              <a-form-model-item :label="$t('chart.axis_value_max')" class="form-item">
                <a-input v-model="axisForm.axisValue.max" @blur="changeYAxisStyle('axisValue')" />
              </a-form-model-item>
              <a-form-model-item :label="$t('chart.axis_value_split')" class="form-item">
                ...
                <a-input v-model="axisForm.axisValue.split" @blur="changeYAxisStyle('axisValue')" />
              </a-form-model-item>
            </span>
          </span>
          ...
          <a-form-model-item v-show="showProperty('splitLine')" :label="$t('chart.axis_show')" class="form-item">
            <a-checkbox v-model="axisForm.axisLine.show" @change="changeYAxisStyle('axisLine')">{{ $t('chart.axis_show')
            }}</a-checkbox>
          </a-form-model-item>
          <a-form-model-item v-show="showProperty('splitLine')" :label="$t('chart.grid_show')" class="form-item">
            <a-checkbox v-model="axisForm.splitLine.show" @change="changeYAxisStyle('splitLine')">{{
              $t('chart.grid_show') }}</a-checkbox>
          </a-form-model-item>
          <span v-show="axisForm.splitLine.show">
            <a-form-model-item v-show="showProperty('splitLine')" :label="$t('chart.grid_color')" class="form-item">
              <vue-color-picker v-model="axisForm.splitLine.lineStyle.color" class="vue-color-picker"
                :predefine="predefineColors" @change="changeYAxisStyle('splitLine')" />
            </a-form-model-item>
            <a-form-model-item v-show="showProperty('splitLine')" :label="$t('chart.grid_width')"
              class="form-item form-item-slider">
              <a-slider v-model="axisForm.splitLine.lineStyle.width" :min="1" :max="10" show-input
                :show-input-controls="false" input-size="mini" @change="changeYAxisStyle('splitLine')" />
            </a-form-model-item>
            <a-form-model-item v-show="showProperty('splitLine')" :label="$t('chart.grid_type')" class="form-item">
          ...
          <a-form-model-item v-show="showProperty('axisLabel')" :label="$t('chart.axis_label_show')" class="form-item">
            <a-checkbox v-model="axisForm.axisLabel.show" @change="changeYAxisStyle('axisLabel')">{{
              $t('chart.axis_label_show') }}</a-checkbox>
          </a-form-model-item>
          <span v-show="axisForm.axisLabel.show">
            <a-form-model-item v-show="showProperty('axisLabel')" :label="$t('chart.axis_label_color')" class="form-item">
            ...
            <a-form-model-item v-show="showProperty('axisLabel')" :label="$t('chart.axis_label_rotate')"
              class="form-item form-item-slider">
              <a-slider v-model="axisForm.axisLabel.rotate" show-input :show-input-controls="false" :min="-90" :max="90"
                input-size="mini" @change="changeYAxisStyle('axisLabel')" />
            </a-form-model-item>
            <a-form-model-item v-show="showProperty('axisLabel')" :label="$t('chart.axis_label_fontsize')"
              class="form-item">
              <a-select v-model="axisForm.axisLabel.fontSize" :placeholder="$t('chart.axis_label_fontsize')"
                dropdownClassName="props-dropdown" @change="changeYAxisStyle('axisLabel')">
                <a-select-option v-for="option in fontSize" :key="option.value" :title="option.name"
                  :value="option.value">{{ option.name }}</a-select-option>
              </a-select>
            </a-form-model-item>
            ...
            <span v-show="chart.type && !chart.type.includes('horizontal')">
              <a-form-model-item :label="$t('chart.value_formatter_type')" class="form-item">
                <a-select v-model="axisForm.axisLabelFormatter.type" @change="changeYAxisStyle('axisLabelFormatter')"
              ...
              <a-form-model-item v-show="axisForm.axisLabelFormatter.type !== 'percent'"
                ...
                <a-select v-model="axisForm.axisLabelFormatter.unit" :placeholder="$t('chart.pls_select_field')"
                  dropdownClassName="props-dropdown" size="small" @change="changeYAxisStyle('axisLabelFormatter')">
              ...
              <a-form-model-item :label="$t('chart.value_formatter_suffix')" class="form-item">
                <a-input v-model="axisForm.axisLabelFormatter.suffix" size="small" clearable
                  :placeholder="$t('commons.input_content')" @change="changeYAxisStyle('axisLabelFormatter')" />
              </a-form-model-item>
...
<script>
import { COLOR_PANEL, DEFAULT_YAXIS_STYLE } from '@/pages/smart-page/util/chart'
