<template>
  <div ref="tableContainer" :style="bg_class" style="padding: 8px;width: 100%;height: 100%;overflow: hidden;">
    ...
    <a-row style="display: flex;flex-direction: column;" :style="cssVars">
      <div style="height:calc(100% - 36px);flex: 1 1 auto;">
      ...
      <vxe-pager style="flex: 0 0 auto;background-color: transparent;" :current-page.sync="currentPage.page"
        :page-size.sync="currentPage.pageSize" :total="currentPage.total" size="mini" @page-change="onPageChange" />
    </a-row>
  </div>
</template>
    ...
    initData() {
      ...
      if (this.chart.data) {
        ...
        if (this.chart.type === 'table-info' && attr.size.tablePageMode === 'page' && this.chart.data.total > this.currentPage.pageSize) {
          this.currentPage.total = Number(this.chart.data.total)
          this.showPage = true
        }
