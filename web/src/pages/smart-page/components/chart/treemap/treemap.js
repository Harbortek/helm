...
export function baseTreemapOption(chart_option, chart) {
  ...
  if (chart.data) {
    ...
    if (chart.data.series.length > 0) {
      ...
      // size
      if (customAttr.size) {
        chart_option.series[0].width =
          (customAttr.size.treemapWidth ? customAttr.size.treemapWidth : 80) +
          "%";
        chart_option.series[0].height =
          (customAttr.size.treemapHeight ? customAttr.size.treemapHeight : 80) +
          "%";
      }
      // label
      if (customAttr.label) {
