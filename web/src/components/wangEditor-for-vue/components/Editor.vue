<script>
import Vue from 'vue';
import { createEditor, SlateEditor, SlateTransforms, SlateNode } from "@wangeditor/editor";
...
export default Vue.extend({
  ...
  watch: {
    ...
    value(newVal) {
      ...
      // 重置 HTML
      console.log('setHtml', newVal, JSON.parse(newVal));
      ...
      createEditor({
        ...
        config: {
          ...
          onChange: (editor) => {
            ...
            this.curValue = JSON.stringify(editor.children); // 记录当前 html 内容
            this.$emit('input', this.curValue); // 用于自定义 v-model
