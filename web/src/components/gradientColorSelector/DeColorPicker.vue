<template>
  <div class="c__color-picker">
    ...
    <a-popover v-model="visible" placement="bottom" title="" width="314" poper-class="" trigger="click" @hide="whenHide">
      ...
      <div v-show="visible" class="">
        ...
        <!-- 画布 -->
        <div class="c__canvas">
          <canvas :id="canvasId" :width="width" :height="height" @click="onCanvasClick" @mousedown="onCanvasMousedown" />
          <!-- 当前选中小块 -->
          <em :id="curId" class="c__cur" @mousedown="function (e) { onCanvasMousedown(e, 'cur') }" />
          <!-- 左侧色条选中小块 -->
          <em :id="barId" class="c__bar" @mousedown="function (e) { onCanvasMousedown(e, 'bar') }" />
          <!-- 透明度控制条 -->
          <div v-if="showAlpha" class="c__alpha-silder">
            <div class="c__alpha-silder-bar" :style="alphaBarStyle" @click="onAlphaClick" @mousedown="onAlphaMousedown" />
            <em :id="alphaId" class="c__alpha" :style="{ left: alpha * 100 + '%' }" @mousedown="onAlphaMousedown" />
          </div>
        </div>
        <!-- 底部按钮栏 -->
      ...
      <!-- 颜色显示方块 -->
      <div slot="reference" class="c__block" :style="{ background: formatBgColor(value) }" />
    </a-popover>
  </div>
</template>
