  ...
  <div>
    <div v-if="colorDto.value" class="color-div-base selected-div">
      <div style="float: left;display: flex;align-items: center;">
        <span v-for="(c, index) in colorDto.colors" :key="index" class="color-span-base"
          :style="{ background: formatBgColor(c, true) }" />
      </div>
      <span style="margin-left: 4px;" />
      <span class="reset-span" @click="reset">
        <a-tooltip class="item" effect="dark" :content="$t('commons.reset')" placement="top">
          <a-icon type="sync" />
        </a-tooltip>
      </span>
    </div>
    <a-popover placement="bottom" popper-class="gradient-popper" :width="popWidth" :append-to-body="true" trigger="click"
      @show="whenShow">
      <template slot="content">
        <div class="custom-switch-div">
          <a-switch v-model="enableCustom" :active-text="$t('commons.custom')" inactive-text="" />
        </div>
        <a-tabs v-model="activeName" @tab-click="handleClick">
          <a-tab-pane
            ...
            :key="i" :tab="pane.label" :name="pane.name">
            <div :id="'color-tab-content-' + i" class="color-tab-content" @click="handler">
              <div v-for="option in pane.data" :key="option.value" class="a-select-dropdown__item color-div-base"
                :class="option.value === colorDto.value ? 'selected hover editor' : ''" @click="selectNode(option)">
                <div v-if="option.value !== colorDto.value || !enableCustom"
                  style="float: left;display: flex;align-items: center;">
                  <span v-for="(c, index) in option.colors" :key="index" class="color-span-base"
                    :style="{ background: formatBgColor(c) }" />
                </div>
                <span v-if="option.value !== colorDto.value || !enableCustom" style="margin-left: 4px;">{{ option.name
                }}</span>

                <span v-for="(co, index) in option.colors" v-else :key="index" class="color-span-base"
                  :class="option.value.endsWith('_split_gradient') && index % 8 !== 0 ? 'static-editor' : 'is-editor'">
                  <span v-if="option.value.endsWith('_split_gradient') && index % 8 !== 0" class="color-span-base-split"
                    :style="{ background: formatBgColor(co) }" />

                  <de-color-picker v-else-if="option.value.endsWith('_continuous_gradient')" :id="option.value + index"
                    ref="de-color-picker" v-model="option.colors[index]" :base-id="option.value + index" show-alpha
                    color-format="rgb" @change="switchColorItem(option.colors, option.value)" />
                  <el-color-picker v-else v-model="option.colors[index]" popper-class="gradient-picker-dropdown"
      ...
      <a-select ref="colorSelect" v-model="colorDto.value" class="form-gender-select" popper-class="gradient-tree-select"
        :placeholder="$t('commons.please_select')" @focus="_popoverShowFun">
        <a-select-option v-for="item in selectOptions" :key="item.label" :title="item.label" :value="item.id">{{
...
export default {
  ...
  methods: {
    ...
    handler(e) {
      const whiteClassName = 'c__block a-popover__reference'
